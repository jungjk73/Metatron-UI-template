(function () {'use strict';
	...
	function getLinkFunction($window, $http, theme, util, type) {
		return function (scope, element, attrs) {
			...
			function setOptions() {
				...
				// 화면 resize 시에 차트 다시 그림
				angular.element($window).on('resize', function () {
					if (chart) {
						chart.resize({
							width: $(ndParent).width(),
							height: $(ndParent).height()
						});
					}
				});

				$('header').bind('headerResizeEvent',function(){
					if (chart) {
						chart.resize({
							width: $(ndParent).width(),
							height: $(ndParent).height()
	...
	var types = ['line', 'bar', 'area', 'pie', 'donut', 'gauge', 'map', 'radar', 'heatmap', 'treemap'];
	for (var i = 0, n = types.length; i < n; i++) {
		(function (type) {
			app.directive(type + 'Chart', ['$window','$http', 'theme', 'util', function ($window, $http, theme, util) {
				return {
					restrict: 'EA',
					...
					link: getLinkFunction($window, $http, theme, util, type)
				};
			}]);
		}(types[i]));
	}
	 ...
	 * util services
	 */
	angular.module('angular-echarts.util', []).factory('util', function ($filter) {
		...
		function getSeries(data, config, type) {
			var series = [];
			angular.forEach(config.series, function (__series, __idx) {

				let __data = data[__idx] || [];
				let __conf = {
					type: type || 'line',
					name: __series.name,
					data: __data
				};
				__conf = angular.extend(__conf, __series);

				if (isAxisChart(type)) {
					// Data 옵션 할당
					angular.forEach(__data, function(_item){
						if (_item.color) {
							_item = angular.extend(_item, {
								itemStyle : {
									normal: {
										color: _item.color
									}
					...
					__series.symbolSize = 0;
				}

				if (isPieChart(type)) {
					__conf.type = 'pie';
					__conf.center = __series.center || ['50%', '50%'];
					__conf.radius = __series.radius || '60%';
					...
					if (type === 'donut') {
						let unit = __series.unit != null && __series.unit != undefined ? __series.unit : 'TB';
						__conf.radius = __series.radius || ['50%', '70%'];
						__conf = angular.extend(__conf, {
							...
							label : {
								normal : {
									...
									formatter : function(params){
										let arr = [
											'{_value|'+params.name+'}\n{_percent|'+params.percent+'% }\n{_value|'+echarts.format.addCommas(params.value)+' '+unit+'}'
										];
										arr.join('\n');
										return arr;
